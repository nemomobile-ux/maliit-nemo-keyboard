...
void PresageWorker::setLanguage(const QString &language)
{
    ...
    if (m_language != language) {
        ...
        QString dbFileName = QString("/usr/share/presage/database_%1.db").arg(m_language);
        if (QFileInfo::exists(dbFileName)) {
            ...
            QString hunspell_base = "/usr/share/hunspell/" + m_language + ".dic";
            m_presage->config("Presage.Predictors.DefaultHunspellPredictor.DICTIONARYBASE",
                              hunspell_base.toLatin1().constData());

            emit languageChanged();
        } else {
            qWarning() << QString("/usr/share/presage/database_%1").arg(m_language) << " not exists";
...
//        m_presage->forget(word.toStdString());
        qWarning() << Q_FUNC_INFO << "Not implemeted in upstream";
        predict_impl(true);
    }
}
